...
void UDragonIKPhysicsComponent::OnHitCallback(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	{
		if (OtherComp != owning_skeleton && ragdoll_mode == false)
		{

			FName parent_bone = owning_skeleton->GetParentBone(Hit.MyBoneName);
			int parent_bone_index = owning_skeleton->GetBoneIndex(parent_bone);

			int hit_bone_index;
			hit_bone_index = owning_skeleton->GetBoneIndex(Hit.MyBoneName);
